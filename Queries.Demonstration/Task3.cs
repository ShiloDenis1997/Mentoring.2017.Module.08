...
namespace Queries.Demonstration
{
    ...
    public class Task3
    {
        ...
        [Test]
        public void Replace_Product_with_the_same_in_NotShipped_Orders()
        {
            var updatedRows = _connection.OrderDetails.LoadWith(od => od.Order)
                ...
                .Update(od => new OrderDetail
                {
                    ProductId = _connection.Products.First(p => !_connection.OrderDetails.Where(t => t.OrderId == od.OrderId)
                            .Any(t => t.ProductId == p.ProductId)).ProductId
                });
